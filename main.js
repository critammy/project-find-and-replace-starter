...
replaceAllButton.addEventListener('click', function () {
    ...
    let userInputString = findInput.value
    ...
    for ( let rowIndex = 0; rowIndex < rowElements.length; rowIndex += 1) {
        ...
        for (let cellIndex = 0; cellIndex < cellElements.length; cellIndex += 1) {
            let currentCellElement = cellElements[cellIndex]
            console.log(currentCellElement)
            let cellString = currentCellElement.innerHTML
            while (cellString.includes(userInputString)) {
                cellString = cellString.replace(userInputString, replaceInputString)
                currentCellElement.innerHTML = cellString
            }
        }
    }
})
